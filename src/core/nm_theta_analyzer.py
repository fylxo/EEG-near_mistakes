...
This script provides a unified interface for all NM theta analysis methods.
It automatically selects the appropriate implementation based on your requirements:

- Single session + basic analysis → nm_theta_single_basic.py
- Single session + parallel processing → nm_theta_single_parallel.py  
- Single session + vectorized/CWT processing → nm_theta_single_vectorized.py
- Multi-session analysis → nm_theta_multi_session.py
...
def main():
    ...
    freq_max = 45.0              # Maximum frequency (Hz)  
    n_freqs = 35                # Number of logarithmically spaced frequencies
    window_duration = 2.0       # Event window duration (±half around event)
    ...
    if use_command_line:
        ...
        parser = argparse.ArgumentParser(
            ...
            epilog="""
Examples:
  # Single session, basic analysis
  python nm_theta_analyzer.py --mode single --method basic --session_index 0 --roi frontal
  
  # Single session, parallel with threading
  python nm_theta_analyzer.py --mode single --method parallel --parallel_type threading --session_index 0 --roi frontal --n_jobs 4
  
  # Single session, vectorized CWT
  python nm_theta_analyzer.py --mode single --method vectorized --session_index 0 --roi hippocampus --batch_size 8
  
  # Multi-session analysis
  python nm_theta_analyzer.py --mode multi --rat_id 10501 --roi frontal --freq_min 2 --freq_max 8
  
  # Custom channels (electrode numbers 1-32)
  python nm_theta_analyzer.py --mode single --method basic --session_index 0 --roi "2,3,5,7"
            """
        )
