#!/usr/bin/env python3
"""
Test script for EEG analysis package using synthetic data.
Generates synthetic EEG data with known oscillatory patterns and validates
that our analysis functions produce expected results.
"""
...
def test_time_domain_functions(data):
    """Test time-domain analysis functions."""
    print("=" * 50)
    print("TESTING TIME-DOMAIN FUNCTIONS")
    print("=" * 50)
    
    # Test single channel stats
    stats = analyze_eeg_time_domain_channel(data, channel_index=0, plot=False)
...
def test_psd_functions(data):
    ...
    # Find theta peak (should be around 6 Hz)
    theta_idx = np.logical_and(freqs >= 5, freqs <= 7)
    theta_power = np.mean(psd[theta_idx])
    
    # Find alpha peak (should be around 10 Hz) 
...
def test_morlet_functions(data):
    """Test Morlet spectrogram functions."""
    print("=" * 50)
    ...
    # Find burst times (should be around every 10 seconds)
    ...
    print(f"Detected theta burst peaks at times: {burst_peaks[:5]}")  # Show first 5
...
def test_integrated_analysis(data):
    """Test the high-level integrated analysis function."""
    print("=" * 50)
    ...
    result = analyze_eeg_channel(
        ...
        plot_types=[],  # No plots for automated testing
        extras=True
    )
