...
This literature gap highlights a critical challenge: demonstrating frontal theta increases during error processing in rodents would represent a significant finding bridging human and animal models of cognitive control.
...
\citet{burle2005sequential} used concurrent EEG and EMG to test conflict monitoring theory, examining whether response conflict levels trigger sequential control adjustments. While error-related negativity (ERN) occurred after incorrect muscle activations, sequential compatibility effects were independent of conflict level, challenging the idea that conflict per se drives cognitive control adjustments.
...
\item \textbf{Frequency resolution}: 59 frequencies between 3--7~Hz (extracted from the 256 frequencies spanning 1--45~Hz used in the Volitional Stopping analysis)
...
A key methodological difference from the Volitional Stopping paper was the use of pre-event baseline normalization rather than global normalization across each session. This method involved:
...
\begin{itemize}
\item Size 1: 18,374 events (33.3\%)
\item Size 2: 18,297 events (33.2\%)
\item Size 3: 18,448 events (33.5\%)
\end{itemize}
...
The complete analysis pipeline was designed for reproducibility with version control and automated parameter logging. Pipeline validation was performed by replicating results from the Volitional Stopping study using the same dataset, with details provided in Supplementary Methods.
...
The approximately equal distribution across size categories (33.3\%, 33.2\%, 33.5\% for sizes 1-3 respectively) confirmed the effectiveness of the tertile-based classification scheme.
...
\centering
\includegraphics[width=\textwidth]{skull_surface_EEG.png}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[width=\textwidth]{EEG_schematic.png}
\end{minipage}
\caption{\textbf{32-electrode skull-surface EEG array configuration.} Left: Schematic shows the electrode positions and corresponding brain areas for each electrode: frontal regions (mPFC), motor cortex, somatosensory–motor areas, and visual cortex. Electrode positions are referenced to bregma coordinates. Taken from Doutel-Figueira et al. (2024), Figure 2A. Right: EEG electrode layout schematic showing spatial organization of the 32-channel array.}
...
The implemented Python pipeline was validated by replicating results from ``Volitional stopping is preceded by a transient beta oscillation'' \citep{doutel2024volitional} using the same dataset. The validation process involved:
...
\item \textbf{Pattern validation}: Successfully reproduced beta band (15--30~Hz) patterns preceding peak velocity
\item \textbf{Graded relationship confirmation}: Validated beta power scaling with movement intensity across all NM sizes
\end{itemize}

This validation established precedent for graded relationships between oscillatory power and behavioral control demands in this paradigm. While beta oscillations likely reflect motor implementation of stopping (consistent with beta's role in motor control), the current study investigates whether frontal theta oscillations reflect the cognitive control aspect of error detection. This complementary framework suggests successful behavioral control involves both cognitive monitoring (frontal theta) and motor implementation (sensorimotor beta) components.
