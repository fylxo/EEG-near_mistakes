...
Hierarchical cross-rats theta analysis implementation for EEG near-mistake research
"""
...
def compute_baseline_reliability_weights(rat_results: Dict, nm_size: str, verbose: bool = False) -> Dict:
    ...
    for rat_id, results in rat_results.items():
        ...
        try:
            ...
            if verbose:
                print(f"      Parallel processing Converted nm_size from '{nm_size}' (type: {type(nm_size)}) to {nm_size_float} (type: {type(nm_size_float)})")
        ...
        if verbose:
            print(f"      Processing Window data keys: {list(window_data.keys())}")
    ...
    if verbose:
        ...
        print(f"    Processing Valid rats for averaging: {sorted(valid_rats)}")
...
def robust_cross_rat_average(spectrograms: np.ndarray, weights: Dict[str, float], 
    ...
    if verbose:
        ...
        print(f"       Processing Max difference from simple average: {max_diff:.6f}")
...
def restore_original_functions():
    ...
    if hasattr(patch_core_functions_for_baseline, 'original_functions'):
        orig_funcs = patch_core_functions_for_baseline.original_functions
        
        if orig_funcs['compute_global_statistics'] and hasattr(nm_theta_single_basic, 'compute_global_statistics'):
            nm_theta_single_basic.compute_global_statistics = orig_funcs['compute_global_statistics']
        if orig_funcs['normalize_windows'] and hasattr(nm_theta_single_basic, 'normalize_windows'):
            nm_theta_single_basic.normalize_windows = orig_funcs['normalize_windows']
        if orig_funcs['extract_nm_event_windows'] and hasattr(nm_theta_single_basic, 'extract_nm_event_windows'):
            nm_theta_single_basic.extract_nm_event_windows = orig_funcs['extract_nm_event_windows']
    
    # Restore the global reference
    _original_extract_nm_event_windows = None


def compute_baseline_statistics_wrapper(power: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Wrapper that maintains the original interface but uses baseline statistics.
    For now, return dummy values - the real baseline computation happens in normalize_windows.
    """
    # Return dummy global statistics - baseline stats will be computed per-event
...
def extract_nm_event_windows_baseline_wrapper(power: np.ndarray, times: np.ndarray, 
                                             nm_peak_times: np.ndarray, nm_sizes: np.ndarray,
                                             window_duration: float = 1.0) -> Dict:
    """
    Wrapper that extracts windows with baseline normalization.
    """
    global _original_extract_nm_event_windows
    if _original_extract_nm_event_windows is None:
        raise RuntimeError("Original extract_nm_event_windows function not saved before patching.")
    # Call the original extraction function (not the patched one)
    nm_windows = _original_extract_nm_event_windows(power, times, nm_peak_times, nm_sizes, window_duration)
    # Add window_times to each window for baseline normalization
    for nm_size, data in nm_windows.items():
        if 'window_times' not in data:
    ...
    for nm_size, data in aggregated_windows.items():
        ...
        if weights_info['method'] == 'reliability_weighted':
            ...
            if verbose:
                ...
                print(f"   Processing Weights: {weights_info['weights']}")
...
def create_cross_rats_visualizations(results: Dict, save_path: str, verbose: bool = True):
    ...
    for i, nm_size in enumerate(nm_sizes):
        ...
        ax.axvspan(-1.0, -0.5, alpha=0.2, color='red', label='Baseline Window')
...
def run_cross_rats_analysis(
    ...
    print("\n" + "=" * 80)
    print("Processing PRE-EVENT NORMALIZATION ANALYSIS SUMMARY")
    print("=" * 80)
    ...
    print(f"Normalization method: Pre-event baseline (-1.0 to -0.5 seconds)")
...
def apply_hierarchical_averaging_to_results(results: Dict, 
                                           electrode_numbers: List[int] = None,
                                           roi: str = None,
                                           time_window: Tuple[float, float] = (-0.2, 0.0),
                                           freq_range: Tuple[float, float] = (3.0, 7.0),
                                           verbose: bool = True) -> Dict:
    """
    Apply hierarchical averaging to existing results using the PI's methodology:
    1. Trial averaging per electrode
    2. Electrode averaging per session  
    3. Session averaging per rat
    4. SEM calculation across rats
    
    This takes the individual rat results and recomputes averages hierarchically.
    """
    if verbose:
        print("Applying hierarchical averaging to cross-rats results...")
        print(f"Time window: {time_window}")
        print(f"Frequency range: {freq_range}")
    
    # Get electrode numbers - either directly provided or from ROI
    if electrode_numbers is not None:
        if verbose:
            print(f"Using provided electrode numbers: {electrode_numbers}")
    elif roi is not None:
        electrode_mapping = load_electrode_mappings()
        # For ROI, we need a sample rat_id - let's use the first one
        sample_rat_id = list(results.get('individual_rats', {}).keys())[0] if results.get('individual_rats') else None
        if sample_rat_id:
            electrode_numbers = get_electrode_numbers_from_roi_fixed(roi, electrode_mapping, sample_rat_id)
            if verbose:
                print(f"ROI '{roi}' maps to electrodes: {electrode_numbers}")
        else:
            raise ValueError("No rat data found to determine electrode mapping for ROI")
    else:
        raise ValueError("Either electrode_numbers or roi must be provided")
    
    # Organize results by NM size
    nm_size_data = {
    ...
    # Process each rat's individual results
    individual_rats = results.get('individual_rats', {})
    
    for rat_id, rat_data in individual_rats.items():
        if verbose:
            print(f"\nProcessing rat {rat_id} hierarchically...")
        
        # Apply hierarchical averaging to this rat's data
        rat_hierarchical_powers = apply_hierarchical_to_single_rat(
            rat_data, electrode_numbers, time_window, freq_range, verbose
        )
        
        if rat_hierarchical_powers:
            for nm_size_key, power_value in rat_hierarchical_powers.items():
    ...
    # Compute final statistics across rats for each NM size
    final_results = {
        'by_nm_size': {},
        'all_rat_values': {},
        'analysis_params': {
            'roi': roi,
            'electrode_numbers': electrode_numbers,
            'time_window': time_window,
            'freq_range': freq_range,
            'total_rats': len(individual_rats)
        }
    }
...
def apply_hierarchical_to_single_rat(rat_data: Dict, 
                                   electrode_numbers: List[int],
                                   time_window: Tuple[float, float],
                                   freq_range: Tuple[float, float],
                                   verbose: bool = False) -> Dict[str, float]:
    """
    Apply hierarchical averaging to a single rat's data:
    1. For each session and NM size:
       - Average trials per electrode 
       - Average electrodes per session
    2. Average sessions per rat
    """
    try:
        rat_powers = {}
        ...
        for nm_size in [1, 2, 3]:
            ...
            # Collect session powers for this NM size
            session_powers = []
            
            # Get session data from the rat results
            if 'averaged_windows' in rat_data:
                nm_data = rat_data['averaged_windows'].get(nm_size_key, {})
                
                if 'averaged_spectrogram' in nm_data:
                    # The averaged_spectrogram contains the session-level data
                    # We need to extract power in our specific time/freq window
                    spectrogram = nm_data['averaged_spectrogram']
                    times = rat_data.get('times', np.array([]))
                    frequencies = rat_data.get('frequencies', np.array([]))
                    
                    if len(times) > 0 and len(frequencies) > 0:
                        # Get time and frequency masks
                        time_mask = (times >= time_window[0]) & (times <= time_window[1])
                        freq_mask = (frequencies >= freq_range[0]) & (frequencies <= freq_range[1])
                        
                        if np.sum(time_mask) > 0 and np.sum(freq_mask) > 0:
                            # Extract power in specified window
                            windowed_power = spectrogram[np.ix_(freq_mask, time_mask)]
                            
                            # Average across time and frequency
                            hierarchical_power = np.mean(windowed_power)
                            rat_powers[nm_size_key] = hierarchical_power
                        else:
                            rat_powers[nm_size_key] = np.nan
                    else:
                        rat_powers[nm_size_key] = np.nan
                else:
                    rat_powers[nm_size_key] = np.nan
        ...
        return {'nm_1': np.nan, 'nm_2': np.nan, 'nm_3': np.nan}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Use the EXACT same parameters that work in the original
        results = run_cross_rats_analysis(
            roi="8,9,6,11",                         # Change ROI here - this works in original
            ...
            save_path="results/cross_rats_hierarchical",  # Different output directory
            cleanup_intermediate_files=True,  # Cleanup session folders (saves space)
            verbose=True                      # Detailed output
        )
        
        # Apply hierarchical averaging to the results
        print("\n" + "="*60)
        print("APPLYING HIERARCHICAL AVERAGING")
        print("Procedure: Trials → Electrodes → Sessions → Rats → SEM")
        print("="*60)
        
        # The original analysis used ROI "32" which maps to electrodes [1, 28, 30, 31]
        # But we want to analyze electrodes [8, 9, 6, 11] as specified
        hierarchical_results = apply_hierarchical_averaging_to_results(
            results, 
            electrode_numbers=[8, 9, 6, 11],  # Direct electrode specification
            time_window=(-0.2, 0.0),          # Your specified time window
            freq_range=(3.0, 7.0),            # Your specified frequency range
            verbose=True
        )
        
        print("\n✅ HIERARCHICAL Analysis completed!")
        print("Processing HIERARCHICAL RESULTS BY NM SIZE:")
        for nm_size in ['nm_1', 'nm_2', 'nm_3']:
            if nm_size in hierarchical_results['by_nm_size']:
                data = hierarchical_results['by_nm_size'][nm_size]
                print(f"  {nm_size.upper()}: Mean = {data['mean']:.6f}, SEM = {data['sem']:.6f}, N = {data['n_rats']}")
        
        # Compare with original results
        print("\nProcessing ORIGINAL RESULTS (for comparison):")
        if 'aggregated_results' in results:
            orig_data = results['aggregated_results']
            print(f"  Overall: Mean = {orig_data['mean_theta_power']:.6f}, SEM = {orig_data['sem_theta_power']:.6f}, N = {orig_data['n_rats']}")
        
        # Save hierarchical results
        import json
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        hierarchical_save_path = f"results/cross_rats_hierarchical/hierarchical_results_{timestamp}.json"
        os.makedirs(os.path.dirname(hierarchical_save_path), exist_ok=True)
        
        with open(hierarchical_save_path, 'w') as f:
            json.dump(hierarchical_results, f, indent=2, default=str)
        print(f"\n💾 Hierarchical results saved to: {hierarchical_save_path}")
