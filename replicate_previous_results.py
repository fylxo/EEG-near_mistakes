#!/usr/bin/env python3
"""
Replicate Previous Results Analysis
==================================
...
def load_all_csv_data(csv_exports_dir):
    ...
    electrode_dirs = glob.glob(os.path.join(csv_exports_dir, "electrode_*"))
    
    print(f"Found {len(electrode_dirs)} electrode directories")
    
    for electrode_dir in sorted(electrode_dirs):
        # Extract electrode number from directory name
        electrode_num = electrode_dir.split("electrode_[")[1].split("]")[0]
        
        # Get all CSV files in this electrode directory
        csv_files = glob.glob(os.path.join(electrode_dir, "*.csv"))
        ...
        for csv_file in csv_files:
            try:
                ...
                # Add electrode info if not present
                if 'electrode_number' not in df.columns or df['electrode_number'].isna().any():
