#!/usr/bin/env python3
"""
Cross-Rats NM Theta Analysis - Epoched Approach with Visualization

This script follows the paper's methodology and provides full spectrogram visualization:
...
5. Generate full spectrogram plots for each NM size
...
def process_single_rat_epoched_viz(rat_id: str, roi_channels: Union[str, List[int]],
    ...
    """
    Process a single rat using the epoched approach with full spectrogram data.
    ...
    --------
    results : Dict
        Results for the rat with full spectrogram data
    """
    if verbose:
        print(f"\n🐀 Processing rat {rat_id} - Epoched Approach with Visualization")
    ...
    try:
        ...
        if save_path:
            ...
            results_file = os.path.join(save_path, f'rat_{rat_id}_epoched_viz_results.pkl')
...
def create_epoched_visualization(results: Dict, save_path: str, verbose: bool = True):
    """
    Create comprehensive visualizations for epoched cross-rats results.
    ...
    """
    if verbose:
        print(f"\n📈 Creating epoched cross-rats visualizations")
        print("=" * 60)
    ...
    fig, axes = plt.subplots(n_nm_sizes, 1, figsize=(12, 6 * n_nm_sizes))
    if n_nm_sizes == 1:
        axes = [axes]
    ...
    for i, nm_size in enumerate(nm_sizes):
        ...
        ax.set_title(f'NM Size {nm_size} - Average Across {n_rats} Rats (Epoched Method)\n'
    ...
    plt.suptitle(f'Cross-Rats NM Theta Analysis - Epoched Method\n{roi_str}, {freq_str}', fontsize=14)
    ...
    # Save plot
    plot_file = os.path.join(save_path, 'cross_rats_epoched_spectrograms.png')
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    if verbose:
        print(f"✓ Epoched spectrograms saved to: {plot_file}")
    
    plt.show()


def run_cross_rats_epoched_viz_analysis(roi: str, pkl_path: str = None,
                                       ...
                                       save_path: str = None, verbose: bool = True) -> Dict:
    """
    Run cross-rats analysis using the epoched approach with visualization.
    ...
    --------
    results : Dict
        Cross-rats aggregated results with visualization
    """
    if pkl_path is None:
        pkl_path = DataConfig.get_data_file_path(DataConfig.MAIN_EEG_DATA_FILE)
    if save_path is None:
        save_path = "results/cross_rats_epoched_viz"
    
    if verbose:
        print("🧠 Cross-Rats NM Theta Analysis - Epoched Approach with Visualization")
        print("=" * 80)
        print(f"Method: Paper's methodology (epoch-wise spectrograms)")
    ...
    for rat_id in rat_ids:
        ...
        results = process_single_rat_epoched_viz(
            rat_id=rat_id,
            roi_channels=roi,
            pkl_path=pkl_path,
    ...
    for rat_id, results in valid_results.items():
        ...
        for nm_size, nm_data in results['nm_results'].items():
            aggregated_windows[nm_size]['spectrograms'].append(nm_data['avg_spectrogram'])
            aggregated_windows[nm_size]['total_events'].append(nm_data['n_events'])
            aggregated_windows[nm_size]['n_sessions'].append(nm_data['n_sessions'])
    ...
    for nm_size, data in aggregated_windows.items():
        ...
        final_aggregated_windows[nm_size] = {
            'avg_spectrogram': avg_spectrogram,
            'window_times': first_result['nm_results'][nm_size]['window_times'],
    ...
    aggregated_results = {
        'analysis_type': 'cross_rats_epoched_viz',
        'method': 'epoched_approach_viz',
        ...
        'processing_info': {
            'n_rats_attempted': len(rat_ids),
            'n_rats_successful': len(valid_results),
    ...
    results_file = os.path.join(save_path, 'cross_rats_epoched_viz_aggregated_results.pkl')
    with open(results_file, 'wb') as f:
        pickle.dump(aggregated_results, f)
    
    if verbose:
        print(f"✓ Cross-rats epoched viz results saved to: {results_file}")
    
    # Create visualizations
    create_epoched_visualization(
    ...
    print("\n" + "=" * 80)
    print("📊 EPOCHED VISUALIZATION ANALYSIS SUMMARY")
    print("=" * 80)
    ...
    if len(successful_rats) > 0:
        ...
        for nm_size, data in final_aggregated_windows.items():
            # Compute single value for comparison
            time_averaged = np.mean(data['avg_spectrogram'], axis=1)
            final_value = np.mean(time_averaged)
            print(f"  NM size {nm_size}: {final_value:.6f}")
        
        print(f"\n✅ Epoched visualization analysis completed successfully!")
...
def main():
    """
    Main function for epoched cross-rats analysis with visualization.
    """
    parser = argparse.ArgumentParser(
        description='Cross-Rats NM Theta Analysis - Epoched Approach with Visualization (Paper\'s Method)',
    ...
    parser.add_argument('--save_path', default="results/cross_rats_epoched_viz",
    ...
    # Run the analysis
    return run_cross_rats_epoched_viz_analysis(
        roi=args.roi,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        start_time = time.time()
        results = run_cross_rats_epoched_viz_analysis(
            roi="1,2,3",                        # Test with ROI 1
            pkl_path=data_path,
            freq_range=(3.0, 45.0),           # 3-7Hz like your other analysis
            n_freqs=30,                      # Same as your other analysis
            ...
            rat_ids=None,                    # Process all rats
            save_path="results/cross_rats_epoched_viz_test",
            verbose=True
        )
        end_time = time.time()